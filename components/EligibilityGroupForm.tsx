...
import { 
  X, Users, Info, Plus, Search, ChevronDown, 
  Trash2, ShieldCheck, UserCheck, AlertTriangle, 
  Settings2, LayoutList, CheckCircle2, ChevronRight,
  Clock
} from 'lucide-react';
...
export const EligibilityGroupForm: React.FC<EligibilityGroupFormProps> = ({ isOpen, onClose, initialData }) => {
  const [activeSection, setActiveSection] = useState<'info' | 'criteria' | 'types' | 'preview'>('info');
  ...
  const [previewResults, setPreviewResults] = useState<{ count: number; samples: any[] } | null>(null);
      ...
      <div className="relative bg-white rounded-[40px] w-full max-w-[650px] max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in duration-300">
        ...
        <div className="bg-white px-10 py-8 border-b border-gray-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-bold text-gray-900">{initialData ? 'Edit Group' : 'Create Eligibility Group'}</h3>
              <p className="text-sm text-gray-400 font-medium">Define automated inclusion rules for leave policies.</p>
            </div>
          </div>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
              ...
              <div className="bg-indigo-50 p-6 rounded-[24px] border border-indigo-100 flex items-center justify-between">
                 <div className="space-y-1">
                   <p className="text-sm font-bold text-indigo-900">Processing Priority</p>
                   <p className="text-xs text-indigo-700 opacity-70">Used when employees fall into multiple groups.</p>
                 </div>
                 <div className="flex items-center gap-4 bg-white p-2 rounded-xl border border-indigo-200">
                    <button className="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 font-bold hover:bg-indigo-100">-</button>
                    <span className="text-lg font-bold text-indigo-900 w-8 text-center">{formData.priority}</span>
                    <button className="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 font-bold hover:bg-indigo-100">+</button>
                 </div>
              </div>
            </div>
            ...
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-300">
              <section className="space-y-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h4 className="text-sm font-bold text-gray-800">Employment Filter</h4>
                </div>
                  ...
                  {employmentTypes.map(type => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setFormData({...formData, criteria: {...formData.criteria, employmentTypes: types}});
                      }}
              ...
              <section className="space-y-4">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h4 className="text-sm font-bold text-gray-800">Grade / Level Range</h4>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-gray-400 uppercase">From Grade</p>
                  ...
                  <div className="space-y-1">
                    <p className="text-[10px] font-bold text-gray-400 uppercase">To Grade</p>
                    <select className="w-full p-3 bg-gray-50 rounded-xl text-sm font-bold outline-none border border-transparent focus:border-gray-200">
                      {grades.map(g => <option key={g} defaultValue={g === 'L5' ? 'L5' : ''}>{g}</option>)}
                    </select>
                  </div>
                </div>
              </section>
              ...
              <div className="grid grid-cols-2 gap-8">
                <section className="space-y-4">
                  ...
                  <div className="flex gap-1 p-1 bg-gray-100 rounded-xl">
                    {['Any', 'Male', 'Female'].map(g => (
                      <button 
                        key={g}
                        onClick={() => setFormData({...formData, criteria: {...formData.criteria, gender: g}})}
                        className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all ${
                          formData.criteria.gender === g ? 'bg-white text-[#3E3B6F] shadow-sm' : 'text-gray-400'
                        }`}
                      >
                        {g}
                      </button>
                ...
                <section className="space-y-4">
                  ...
                  <select 
                    ...
                    onChange={(e) => setFormData({...formData, criteria: {...formData.criteria, probationStatus: e.target.value}})}
                  >
                    <option>Include</option>
                    <option>Exclude</option>
                    <option>Only Probation</option>
                  </select>
                </section>
              </div>
          ...
          {activeSection === 'types' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
               <div className="flex items-center justify-between p-6 bg-[#3E3B6F] rounded-3xl text-white shadow-xl">
                 <div className="flex items-center gap-4">
                   <div className="p-3 bg-white/10 rounded-2xl"><LayoutList size={24}/></div>
                   <div>
                     <h5 className="font-bold">Assign All Leave Types?</h5>
                     <p className="text-xs text-white/50">Grant access to every current and future leave type.</p>
                   </div>
                 </div>
                 <button 
                  onClick={() => setFormData({...formData, allTypes: !formData.allTypes})}
                  className={`w-12 h-6 rounded-full relative transition-colors ${formData.allTypes ? 'bg-[#E8D5A3]' : 'bg-white/20'}`}
                 >
                   <div className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${formData.allTypes ? 'left-7' : 'left-1'}`} />
                 </button>
               </div>
               ...
               {!formData.allTypes && (
                 <div className="grid grid-cols-2 gap-4">
                    {leaveTypes.map(type => (
                      <label key={type.id} className={`flex items-center justify-between p-5 border-2 rounded-2xl cursor-pointer transition-all ${
                        formData.leaveTypes.includes(type.id) ? 'bg-indigo-50 border-[#3E3B6F]' : 'bg-white border-gray-100 hover:border-gray-200'
                      }`}>
                        <span className={`text-sm font-bold ${formData.leaveTypes.includes(type.id) ? 'text-[#3E3B6F]' : 'text-gray-600'}`}>{type.label}</span>
                        <input 
                          type="checkbox" 
               ...
               {previewResults ? (
                 <>
                   <div className="p-8 bg-emerald-50 rounded-[32px] border border-emerald-100 text-center space-y-4">
                      ...
                      <div>
                        <h4 className="text-3xl font-bold text-emerald-900">{previewResults.count}</h4>
                        <p className="text-sm font-bold text-emerald-700 uppercase tracking-widest">Matching Employees</p>
                      </div>
                   </div>

                   <div className="space-y-4">
                      <h5 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Sample List (Top 5)</h5>
                      <div className="space-y-3">
                        {previewResults.samples.map((s, i) => (
                          <div key={i} className="flex items-center justify-between p-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
                            ...
                            <span className="text-[10px] font-bold text-gray-400 uppercase">{s.dept}</span>
                          </div>
                        ))}
                      </div>
                 ...
                 <div className="py-20 text-center space-y-6">
                    ...
                    <div>
                      <h4 className="text-lg font-bold text-gray-800">Ready to test?</h4>
                      <p className="text-sm text-gray-400">Click the button below to see which employees currently match your criteria.</p>
                    </div>
                    <button 
        ...
        <div className="bg-gray-50 px-10 py-8 border-t border-gray-100 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2 text-amber-600">
             ...
             <p className="text-[10px] font-bold uppercase tracking-widest">Priority overlaps existing groups</p>
          </div>
          <div className="flex gap-4">
            <button onClick={onClose} className="px-6 py-2.5 text-gray-500 font-bold hover:bg-gray-100 rounded-xl transition-all">Cancel</button>
            <button 
              onClick={onClose}
              className="px-8 py-2.5 bg-[#3E3B6F] text-white font-bold rounded-xl hover:bg-[#4A4680] shadow-lg shadow-[#3E3B6F]/20 transition-all active:scale-95"
            >
              Save Group Settings
            </button>
          </div>
