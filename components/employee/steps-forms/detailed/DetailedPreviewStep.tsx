// src/features/enrollment/steps/DetailedPreviewStep.tsx
"use client";
...
const DetailedPreviewStep = forwardRef<StepHandle, StepComponentProps>(function DetailedPreviewStep(
  ...
  }, [isBusy, enrollmentId, confirm, clearEnrollment, navigate]);
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="text-xl font-extrabold leading-tight">Confirm & Submit</div>
            <div className="text-[12px] opacity-90 mt-1">Enrollment #{enrollmentId ?? "—"}</div>
          </div>

          <div className="rounded-xl bg-white/10 border border-white/15 p-2">
            <div className="flex items-center gap-2">
              <ShieldCheck className="w-4 h-4" style={{ color: colors.beige }} />
        ...
        <div className="mt-3 rounded-xl bg-white/10 border border-white/15 p-3">
          <div className="flex items-start gap-2">
            <AlertTriangle className="w-4 h-4 mt-0.5" style={{ color: colors.beige }} />
            <div className="text-[12px] leading-relaxed opacity-95">
              Once you submit, this enrollment will be <b>finalized</b>. If anything is wrong, go back and fix it
              before submitting.
            </div>
          </div>
        </div>
      ...
      {/* hidden button for safety */}
      <button type="button" className="hidden" aria-hidden="true" />
    </div>
  );
});

export default DetailedPreviewStep;
