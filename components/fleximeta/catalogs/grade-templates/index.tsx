...
import {
  ...
  useDeleteGradeTemplate,
} from "./useGradeTemplates";
import {
  flexRender,
  getCoreRowModel,
  useReactTable,
...
import type { GradeTemplateRow, FrontendGradeTemplatesResponse } from "./types";
import type { UseQueryResult } from "@tanstack/react-query";
import { toast } from "sonner";
import { CreateGradeTemplateModal } from "./CreateGradeTemplateModal";
import { EditGradeTemplateModal } from "./EditGradeTemplateModal";
import { DeleteGradeTemplateDialog } from "./DeleteGradeTemplateDialog";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
const GradeTemplateListing: React.FC = () => {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<GradeTemplateRow | null>(null);
  const [templateToDelete, setTemplateToDelete] = useState<GradeTemplateRow | null>(null);
  ...
  const {
    ...
    isFetching,
    refetch,
  }: UseQueryResult<FrontendGradeTemplatesResponse, Error> = useGradeTemplates(
    page,
    perPage,
    q
  );

  const createTemplateMutation = useCreateGradeTemplate();
  const updateTemplateMutation = useUpdateGradeTemplate();
  const deleteTemplateMutation = useDeleteGradeTemplate();

  React.useEffect(() => {
    setPage(1);
  }, [perPage, q]);

  const rows: GradeTemplateRow[] = React.useMemo(() => {
    if (!data) return [];
  ...
  const handleEditSubmit = async (values: any) => {
    ...
    try {
      ...
      toast.success("Grade template updated successfully");
      setIsEditModalOpen(false);
      setSelectedTemplate(null);
      refetch();
    } catch (error) {
      toast.error("Failed to update grade template");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success("Grade template deleted successfully");
      setIsDeleteDialogOpen(false);
      setTemplateToDelete(null);
      refetch();
    } catch (error) {
      toast.error("Failed to delete grade template");
    }
  };

  const openEditModal = (template: GradeTemplateRow) => {
    setSelectedTemplate(template);
    setIsEditModalOpen(true);
  };

  const openDeleteDialog = (template: GradeTemplateRow) => {
  ...
  const columns: ColumnDef<GradeTemplateRow>[] = React.useMemo(
    () => [
      {
        accessorKey: "sr",
        header: "Sr. No.",
      ...
      },
      {
        ...
        header: "Components",
        cell: ({ row }) => renderComponents(row.original.payload_json.components),
      },
      {
        accessorKey: "active",
        header: "Status",
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {totalEntries.toLocaleString()} templates registered •
...
export default GradeTemplateListing;