...
import type { SalaryComponentRow, FrontendSalaryComponentsResponse } from "./types";
import type { UseQueryResult } from "@tanstack/react-query";
import { toast } from "sonner";
import { CreateSalaryComponentModal } from "./CreateSalaryComponentModal";
import { EditSalaryComponentModal } from "./EditSalaryComponentModal";
import { DeleteSalaryComponentDialog } from "./DeleteSalaryComponentDialog";
...
const SalaryComponentsListing: React.FC = () => {
  ...
  const [componentToDelete, setComponentToDelete] = useState<SalaryComponentRow | null>(null);
  ...
  const {
    ...
    isFetching,
    refetch,
  }: UseQueryResult<FrontendSalaryComponentsResponse, Error> = useSalaryComponents(
    page,
    perPage,
    q,
    typeFilter,
  ...
  const deleteSalaryComponentMutation = useDeleteSalaryComponent();

  // reset to page 1 when perPage or q changes
  React.useEffect(() => {
    setPage(1);
  }, [perPage, q, typeFilter, activeFilter]);

  const rows: SalaryComponentRow[] = React.useMemo(() => {
    if (!data) return [];
    return data.data.map((component, index) => ({
      ...
      formula_hint: component.formula_hint,
  ...
  const handleCreateSubmit = async (values: any) => {
    try {
      await createSalaryComponentMutation.mutateAsync(values);
  ...
  // Open edit modal
  const openEditModal = (component: SalaryComponentRow) => {
    setSelectedComponent(component);
    setIsEditModalOpen(true);
  };

  // Open delete dialog
  const openDeleteDialog = (component: SalaryComponentRow) => {
  ...
  const getTypeLabel = (type: string) => {
    const typeOption = SALARY_COMPONENT_TYPE_OPTIONS.find(t => t.value === type);
    return typeOption?.label || type;
  };

  // Get formula hint label
  const getFormulaHintLabel = (hint: string) => {
    const hintOption = FORMULA_HINT_OPTIONS.find(h => h.value === hint);
    return hintOption?.label || hint;
  };

  // Define columns
  const columns: ColumnDef<SalaryComponentRow>[] = React.useMemo(
    () => [
      {
        accessorKey: "sr",
        header: "Sr. No.",
      ...
      },
      {
        ...
        header: "Type",
        cell: ({ row }) => {
          const typeColor = row.original.type === "earning" ? "green" : 
                           row.original.type === "deduction" ? "red" : "blue";
          return (
            <Badge
              variant="outline"
              className={`border-${typeColor}-200 bg-${typeColor}-50 text-${typeColor}-700 capitalize`}
            >
  ...
  return (
    <div className="p-6">
      ...
      <CreateSalaryComponentModal
        ...
        isLoading={createSalaryComponentMutation.isPending}
      />
      ...
      <EditSalaryComponentModal
        ...
        isLoading={updateSalaryComponentMutation.isPending}
      ...
      <DeleteSalaryComponentDialog
        ...
        componentInfo={`${componentToDelete?.code} - ${componentToDelete?.name}`}
        isLoading={deleteSalaryComponentMutation.isPending}
      />
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {totalEntries.toLocaleString()} salary components • Configure earnings, deductions, and allowances
              </p>
            </div>
            <div className="flex items-center gap-2">
              <Button
        ...
        <CardContent className="p-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search by code, name, or formula hint..."
                value={search}
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="text-sm">
                ...
                <span className="text-gray-600">salary components</span>
              </div>
              {isFetching && (
                <Badge
                  variant="outline"
                    ...
                    <tr>
                      ...
                      >
                        <div className="text-gray-500">
                          <p className="font-medium">No salary components found</p>
...
export default SalaryComponentsListing;