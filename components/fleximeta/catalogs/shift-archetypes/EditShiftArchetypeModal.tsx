...
interface EditShiftArchetypeModalProps {
  open: boolean;
...
export const EditShiftArchetypeModal: React.FC<EditShiftArchetypeModalProps> = ({
  open,
  onOpenChange,
  onSubmit,
  isLoading,
  ...
  // Initialize form data when shiftArchetype changes or modal opens
  useEffect(() => {
    if (shiftArchetype && open) {
      setFormData({
        code: shiftArchetype.code || "",
        name: shiftArchetype.name || "",
        start_time: shiftArchetype.start_time || "09:00",
        end_time: shiftArchetype.end_time || "18:00",
        break_min: shiftArchetype.break_min || 60,
        night_shift: shiftArchetype.night_shift || false,
        ot_rule_json: {
          ot_after_min: shiftArchetype.ot_rule_json?.ot_after_min || 480,
          multiplier: shiftArchetype.ot_rule_json?.multiplier || 1.5
        },
        active: shiftArchetype.active || true,
      });
    }
  }, [shiftArchetype, open]);
...
const formatTimeToHi = (timeString: string) => {
 ...
 const handleSubmit = (e: React.FormEvent) => {
  ...
  // Format time fields to ensure H:i format before submission
  const formattedData = {
    ...formData,
    start_time: formatTimeToHi(formData.start_time),
    end_time: formatTimeToHi(formData.end_time),
  };
  
  // Validate the formatted times

  
  // Also update the form state with formatted values for consistency
      ...
      <DialogContent className="sm:max-w-2xl bg-white">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold flex items-center gap-2">
            ...
            Edit Shift Archetype
          </DialogTitle>
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-sm text-gray-500">
            <div className="flex items-center gap-2">
              <span>ID: {shiftArchetype.id}</span>
              <Badge variant="outline" className="text-xs">
                {shiftArchetype.active ? "Active" : "Inactive"}
              </Badge>
            </div>
            <div className="flex items-center gap-2">
              <CalendarIcon className="h-3 w-3" />
              <span>Created: {new Date(shiftArchetype.created_at).toLocaleDateString()}</span>
            </div>
          </div>
        </DialogHeader>
