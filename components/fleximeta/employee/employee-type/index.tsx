...
import type { EmployeeTypeRow, FrontendEmployeeTypesResponse } from "./types";
import type { UseQueryResult } from "@tanstack/react-query";
import { toast } from "sonner";
import { 
  CreateEmployeeTypeModal, 
  EditEmployeeTypeModal, 
  DeleteEmployeeTypeDialog 
} from "./modals";
...
const EmployeeTypeListing: React.FC = () => {
  ...
  const [selectedEmployeeType, setSelectedEmployeeType] = useState<EmployeeTypeRow | null>(null);
  const [employeeTypeToDelete, setEmployeeTypeToDelete] = useState<EmployeeTypeRow | null>(null);
  ...
  const {
    ...
    isFetching,
    refetch,
  }: UseQueryResult<FrontendEmployeeTypesResponse, Error> = useEmployeeTypes(
    page,
    perPage,
    q
  );
  ...
  const rows: EmployeeTypeRow[] = React.useMemo(() => {
    if (!data) return [];
    return data.data.map((employeeType, index) => ({
  ...
  // Handle create employee type
  const handleCreateSubmit = async (values: any) => {
    try {
      await createEmployeeTypeMutation.mutateAsync(values);
      toast.success("Employee type created successfully");
      setIsCreateModalOpen(false);
      refetch();
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Failed to create employee type");
    }
  };
  ...
  const handleEditSubmit = async (values: any) => {
    if (!selectedEmployeeType) {
      console.error("No selected employee type");
      return;
    }
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to update employee type");
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to delete employee type");
    }
  };

  // Open edit modal
  const openEditModal = (employeeType: EmployeeTypeRow) => {
    console.log("Opening edit modal for:", employeeType);
    setSelectedEmployeeType(employeeType);
    setIsEditModalOpen(true);
  };

  // Open delete dialog
  const openDeleteDialog = (employeeType: EmployeeTypeRow) => {
  ...
  // Define columns
  const columns: ColumnDef<EmployeeTypeRow>[] = React.useMemo(
    () => [
      {
        accessorKey: "sr",
        header: "Sr. No.",
  ...
  return (
    <div className="p-6">
      ...
      <CreateEmployeeTypeModal
        ...
        isLoading={createEmployeeTypeMutation.isPending}
      />
      ...
      <DeleteEmployeeTypeDialog
        ...
        employeeTypeName={employeeTypeToDelete?.name || ""}
        isLoading={deleteEmployeeTypeMutation.isPending}
      />
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {totalEntries.toLocaleString()} employee types registered •
        ...
        <CardContent className="p-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search employee types by name..."
                value={search}
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-gray-600">Loading employee types...</p>
                        </div>
                      </td>
                    </tr>
                  ) : isError ? (
                    ...
                    <tr>
                      ...
                      >
                        <div className="text-gray-500">
                          <p className="font-medium">No employee types found</p>
...
export default EmployeeTypeListing;