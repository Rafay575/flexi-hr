...
import type { GenderRow, FrontendGendersResponse } from "./types";
import type { UseQueryResult } from "@tanstack/react-query";
import { toast } from "sonner";
import { CreateGenderModal, EditGenderModal, DeleteGenderDialog } from "./modals";
...
const GenderListing: React.FC = () => {
  ...
  const [selectedGender, setSelectedGender] = useState<GenderRow | null>(null);
  const [genderToDelete, setGenderToDelete] = useState<GenderRow | null>(null);
  ...
  const {
    ...
    isFetching,
    refetch,
  }: UseQueryResult<FrontendGendersResponse, Error> = useGenders(
    page,
    perPage,
    q
  );
  ...
  const rows: GenderRow[] = React.useMemo(() => {
    if (!data) return [];
    return data.data.map((gender, index) => ({
  ...
  // Handle create gender
  const handleCreateSubmit = async (values: any) => {
    try {
      await createGenderMutation.mutateAsync(values);
      toast.success("Gender created successfully");
      setIsCreateModalOpen(false);
      refetch();
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Failed to create gender");
    }
  };
  ...
  const handleEditSubmit = async (values: any) => {
    if (!selectedGender) {
      console.error("No selected gender");
      return;
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteGenderMutation.mutateAsync(genderToDelete.id);
  ...
  // Open edit modal
  const openEditModal = (gender: GenderRow) => {
    setSelectedGender(gender);
    setIsEditModalOpen(true);
  };

  // Open delete dialog
  const openDeleteDialog = (gender: GenderRow) => {
  ...
  // Define columns
  const columns: ColumnDef<GenderRow>[] = React.useMemo(
    () => [
      {
        accessorKey: "sr",
        header: "Sr. No.",
  ...
  return (
    <div className="p-6">
      ...
      <CreateGenderModal
        ...
        isLoading={createGenderMutation.isPending}
      />
      ...
      <EditGenderModal
        ...
        isLoading={updateGenderMutation.isPending}
      ...
      <DeleteGenderDialog
        ...
        genderName={genderToDelete?.name || ""}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {totalEntries.toLocaleString()} genders registered •
        ...
        <CardContent className="p-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search genders by name..."
                value={search}
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-gray-600">Loading genders...</p>
                        </div>
                      </td>
                    </tr>
                  ) : isError ? (
                    ...
                    <tr>
                      ...
                      >
                        <div className="text-gray-500">
                          <p className="font-medium">No genders found</p>
