...
import type { HealthRow, FrontendHealthsResponse } from "./types";
...
const HealthListing: React.FC = () => {
  ...
  const [selectedHealth, setSelectedHealth] = useState<HealthRow | null>(null);
  const [healthToDelete, setHealthToDelete] = useState<HealthRow | null>(null);
  ...
  const {
    ...
    isFetching,
    refetch,
  }: UseQueryResult<FrontendHealthsResponse, Error> = useHealths(
    page,
    perPage,
    q
  );

  const createHealthMutation = useCreateHealth();
  ...
  const rows: HealthRow[] = React.useMemo(() => {
    if (!data) return [];
    return data.data.map((health, index) => ({
  ...
  // Handle create health condition
  const handleCreateSubmit = async (values: any) => {
    try {
      await createHealthMutation.mutateAsync(values);
      toast.success("Health condition created successfully");
      setIsCreateModalOpen(false);
      refetch();
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Failed to create health condition");
    }
  };

  // Handle edit health condition
  const handleEditSubmit = async (values: any) => {
    if (!selectedHealth) {
      console.error("No selected health condition");
      return;
    }
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to update health condition");
    }
  };

  // Handle delete health condition
  const handleDelete = async () => {
    if (!healthToDelete) return;

    try {
      await deleteHealthMutation.mutateAsync(healthToDelete.id);
      toast.success("Health condition deleted successfully");
      setIsDeleteDialogOpen(false);
      setHealthToDelete(null);
      refetch();
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Failed to delete health condition");
    }
  };

  // Open edit modal
  const openEditModal = (health: HealthRow) => {
    console.log("Opening edit modal for:", health);
    setSelectedHealth(health);
    setIsEditModalOpen(true);
  };

  // Open delete dialog
  const openDeleteDialog = (health: HealthRow) => {
  ...
  // Define columns
  const columns: ColumnDef<HealthRow>[] = React.useMemo(
    () => [
      {
        accessorKey: "sr",
        header: "Sr. No.",
  ...
  return (
    <div className="p-6">
      ...
      <CreateHealthModal
        ...
        isLoading={createHealthMutation.isPending}
      />
      ...
      <DeleteHealthDialog
        ...
        healthName={healthToDelete?.name || ""}
        isLoading={deleteHealthMutation.isPending}
      />
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {totalEntries.toLocaleString()} health conditions registered •
        ...
        <CardContent className="p-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search health conditions by name..."
                value={search}
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-gray-600">Loading health conditions...</p>
                        </div>
                      </td>
                    </tr>
                  ) : isError ? (
                    ...
                    <tr>
                      ...
                      >
                        <div className="text-gray-500">
                          <p className="font-medium">No health conditions found</p>
