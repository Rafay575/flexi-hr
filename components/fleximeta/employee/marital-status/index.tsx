...
import type { MaritalStatusRow, FrontendMaritalStatusesResponse } from "./types";
import type { UseQueryResult } from "@tanstack/react-query";
import { toast } from "sonner";
import { 
  CreateMaritalStatusModal, 
  EditMaritalStatusModal, 
  DeleteMaritalStatusDialog 
} from "./modals";
...
const MaritalStatusListing: React.FC = () => {
  ...
  const [selectedMaritalStatus, setSelectedMaritalStatus] = useState<MaritalStatusRow | null>(null);
  const [maritalStatusToDelete, setMaritalStatusToDelete] = useState<MaritalStatusRow | null>(null);
  ...
  const {
    ...
    isFetching,
    refetch,
  }: UseQueryResult<FrontendMaritalStatusesResponse, Error> = useMaritalStatuses(
    page,
    perPage,
    q
  );

  const createMaritalStatusMutation = useCreateMaritalStatus();
  const updateMaritalStatusMutation = useUpdateMaritalStatus();
  const deleteMaritalStatusMutation = useDeleteMaritalStatus();
  ...
  const rows: MaritalStatusRow[] = React.useMemo(() => {
    if (!data) return [];
    return data.data.map((maritalStatus, index) => ({
  ...
  const handleCreateSubmit = async (values: any) => {
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to create marital status");
    }
  };

  // Handle edit marital status
  const handleEditSubmit = async (values: any) => {
    if (!selectedMaritalStatus) {
      console.error("No selected marital status");
      return;
    }
  ...
  // Handle delete marital status
  const handleDelete = async () => {
    if (!maritalStatusToDelete) return;

    try {
      await deleteMaritalStatusMutation.mutateAsync(maritalStatusToDelete.id);
  ...
  // Open edit modal
  const openEditModal = (maritalStatus: MaritalStatusRow) => {
    console.log("Opening edit modal for:", maritalStatus);
    setSelectedMaritalStatus(maritalStatus);
    setIsEditModalOpen(true);
  };

  // Open delete dialog
  const openDeleteDialog = (maritalStatus: MaritalStatusRow) => {
    setMaritalStatusToDelete(maritalStatus);
  ...
  // Define columns
  const columns: ColumnDef<MaritalStatusRow>[] = React.useMemo(
    () => [
      {
        accessorKey: "sr",
        header: "Sr. No.",
  ...
  return (
    <div className="p-6">
      ...
      <CreateMaritalStatusModal
        ...
        isLoading={createMaritalStatusMutation.isPending}
      />
      ...
      <EditMaritalStatusModal
        ...
        isLoading={updateMaritalStatusMutation.isPending}
      ...
      <DeleteMaritalStatusDialog
        ...
        maritalStatusName={maritalStatusToDelete?.name || ""}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {totalEntries.toLocaleString()} marital statuses registered •
        ...
        <CardContent className="p-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search marital statuses by name..."
                value={search}
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-gray-600">Loading marital statuses...</p>
                        </div>
                      </td>
                    </tr>
                  ) : isError ? (
                    ...
                    <tr>
                      ...
                      >
                        <div className="text-gray-500">
                          <p className="font-medium">No marital statuses found</p>
