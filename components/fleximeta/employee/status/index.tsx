...
import type { EmployeeStatusRow, FrontendEmployeeStatusesResponse } from "./types";
import type { UseQueryResult } from "@tanstack/react-query";
import { toast } from "sonner";
import { 
  CreateEmployeeStatusModal, 
  EditEmployeeStatusModal, 
  DeleteEmployeeStatusDialog 
} from "./modals";
...
const EmployeeStatusListing: React.FC = () => {
  ...
  const [selectedStatus, setSelectedStatus] = useState<EmployeeStatusRow | null>(null);
  const [statusToDelete, setStatusToDelete] = useState<EmployeeStatusRow | null>(null);
  ...
  const {
    ...
    isFetching,
    refetch,
  }: UseQueryResult<FrontendEmployeeStatusesResponse, Error> = useEmployeeStatuses(
    page,
    perPage,
    q
  );

  const createStatusMutation = useCreateEmployeeStatus();
  const updateStatusMutation = useUpdateEmployeeStatus();
  const deleteStatusMutation = useDeleteEmployeeStatus();
  ...
  const rows: EmployeeStatusRow[] = React.useMemo(() => {
    if (!data) return [];
    return data.data.map((status, index) => ({
  ...
  // Handle create employee status
  const handleCreateSubmit = async (values: any) => {
    try {
      await createStatusMutation.mutateAsync(values);
      toast.success("Employee status created successfully");
      setIsCreateModalOpen(false);
      refetch();
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Failed to create employee status");
    }
  };

  // Handle edit employee status
  const handleEditSubmit = async (values: any) => {
    if (!selectedStatus) {
      console.error("No selected employee status");
      return;
    }
  ...
  // Handle delete employee status
  const handleDelete = async () => {
    if (!statusToDelete) return;

    try {
      await deleteStatusMutation.mutateAsync(statusToDelete.id);
      toast.success("Employee status deleted successfully");
      setIsDeleteDialogOpen(false);
      setStatusToDelete(null);
      refetch();
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Failed to delete employee status");
    }
  };

  // Open edit modal
  const openEditModal = (status: EmployeeStatusRow) => {
    console.log("Opening edit modal for:", status);
    setSelectedStatus(status);
    setIsEditModalOpen(true);
  };

  // Open delete dialog
  const openDeleteDialog = (status: EmployeeStatusRow) => {
  ...
  // Define columns
  const columns: ColumnDef<EmployeeStatusRow>[] = React.useMemo(
    () => [
      {
        accessorKey: "sr",
        header: "Sr. No.",
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {totalEntries.toLocaleString()} statuses registered •
        ...
        <CardContent className="p-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search employee statuses by name..."
                value={search}
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-gray-600">Loading employee statuses...</p>
                        </div>
                      </td>
                    </tr>
                  ) : isError ? (
                    ...
                    <tr>
                      ...
                      >
                        <div className="text-gray-500">
                          <p className="font-medium">No employee statuses found</p>
...
export default EmployeeStatusListing;