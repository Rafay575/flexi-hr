...
const BankListing: React.FC = () => {
  ...
  const rows = useMemo(() => {
    if (!data?.data) return [];
    return data.data.map((bank, index) => ({
  ...
  const handleCreateSubmit = async (values: any) => {
    try {
      await createBankMutation.mutateAsync(values);
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteBankMutation.mutateAsync(bankToDelete.id.toString());
  ...
  // Open delete dialog
  const openDeleteDialog = (bank: any) => {
    setBankToDelete(bank);
    setIsDeleteDialogOpen(true);
  };

  // Define columns for Banks
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">{row.original.country_name}</span>
            </div>
            <Badge
              variant="outline"
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {totalEntries.toLocaleString()} banks registered • Server-side
                pagination & search
              </p>
            </div>
            <div className="flex items-center gap-2">
        ...
        <CardContent className="p-6">
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-6">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Search banks by code, name, country..."
                value={search}
