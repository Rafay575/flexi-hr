...
import type { RegionRow, FrontendRegionsResponse } from "./types";
import type { UseQueryResult } from "@tanstack/react-query";
import { toast } from "sonner";
import { CreateRegionModal } from "./modals";
import { EditRegionModal } from "./modals";
import { DeleteRegionDialog } from "./modals";
...
const RegionsListing: React.FC = () => {
  ...
  const [regionToDelete, setRegionToDelete] = useState<RegionRow | null>(null);
  ...
  const {
    ...
    isFetching,
    refetch,
  }: UseQueryResult<FrontendRegionsResponse, Error> = useRegions(
    page,
    perPage,
    q
  );
  ...
  const rows: RegionRow[] = React.useMemo(() => {
    if (!data) return [];
    return data.data.map((region, index) => ({
  ...
  const handleCreateSubmit = async (values: any) => {
    try {
      await createRegionMutation.mutateAsync(values);
  ...
  const handleEditSubmit = async (values: any) => {
    if (!selectedRegion) {
      console.error("No selected region");
      return;
    }
  ...
  // Open edit modal
  const openEditModal = (region: RegionRow) => {
    setSelectedRegion(region);
    setIsEditModalOpen(true);
  };

  // Open delete dialog
  const openDeleteDialog = (region: RegionRow) => {
  ...
  // Define columns
  const columns: ColumnDef<RegionRow>[] = React.useMemo(
    () => [
      {
        accessorKey: "sr",
        header: "Sr. No.",
  ...
  return (
    <div className="p-6">
      ...
      <EditRegionModal
        ...
        isLoading={updateRegionMutation.isPending}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                {totalEntries.toLocaleString()} regions registered •
