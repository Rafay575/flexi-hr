    ...
    events: [
      {
        ...
        description: "Triggered when a new leave application is created.",
      ...
      },
      {
        ...
        defaultRecipients: "Requester",
        status: "configured",
      },
      {
        id: "leave.rejected",
        label: "Request Rejected",
        description: "Sent when leave request is denied.",
        defaultRecipients: "Requester",
        status: "default",
      },
      {
        ...
        description: "Triggered upon cancellation by requester or admin.",
        defaultRecipients: "Approver, HR",
        status: "default",
      },
    ],
  },
    ...
    events: [
      {
        ...
        defaultRecipients: "Employee",
        status: "default",
      },
      {
        id: "balance.expiry",
        label: "Balance Expiry Alert",
        description: "Sent before carry-forward days expire.",
        defaultRecipients: "Employee",
        status: "configured",
      },
    ],
    ...
    events: [
      {
        ...
        label: "SLA Reminder",
        description: "Sent to approver when request is near SLA breach.",
      ...
      },
      {
        ...
        description: "Sent when request breaches SLA time.",
        defaultRecipients: "Manager of Approver, HR",
        status: "default",
      },
    ],
  },
    ...
    events: [
      {
        ...
        description: "Sent when extra work is verified as credit.",
        defaultRecipients: "Employee",
        status: "configured",
      },
    ],
...
export const LeaveNotificationTemplates = () => {
  ...
  const selectedEvent = CATEGORIES.flatMap((c) => c.events).find(
                ...
                {cat.events.map((event) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-2 h-2 rounded-full ${event.status === "configured" ? "bg-emerald-500" : "bg-gray-200"}`}
                        title={event.status}
                      />
                    ...
                    <ChevronRight
                      size={14}
                      className={`text-gray-300 group-hover:translate-x-0.5 transition-transform ${selectedEventId === event.id ? "opacity-100" : "opacity-0"}`}
                    />
                  </button>
                ))}
              </div>
              ...
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400 font-medium italic mt-1">
                    ID: {selectedEvent.id} • {selectedEvent.description}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                >
                  <Eye size={18} />{" "}
                  {isPreviewMode ? "Edit Mode" : "Preview Result"}
                </button>
              ...
              <div className="flex-1 space-y-8">
                    ...
                    {Object.entries(channels).map(([name, enabled]) => (
                      <button
                        ...
                        className={`flex items-center gap-4 p-5 rounded-2xl border-2 transition-all text-left ${
                          activeChannel === name
                            ? "border-[#3E3B6F] bg-indigo-50/30"
                            : "border-gray-50 hover:border-gray-100"
                        }`}
                      >
                        <div
                          className={`w-5 h-5 rounded flex items-center justify-center border-2 transition-all ${enabled ? "bg-[#3E3B6F] border-[#3E3B6F]" : "border-gray-300"}`}
                        >
                          {enabled && (
                            <CheckSquare className="text-[#E8D5A3]" size={14} />
                          )}
                        </div>
                        <div className="flex items-center gap-3">
                          {name === "Email" ? (
                ...
                <div className="bg-white rounded-[40px] border border-gray-100 shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                  <div className="p-8 border-b border-gray-100 bg-gray-50/30 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="px-3 py-1 bg-[#3E3B6F] text-white text-[10px] font-bold uppercase rounded-lg tracking-widest">
                        {activeChannel} Template
                      </div>
                      <p className="text-xs text-gray-400 italic">
                        Editing default version for all staff
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <button className="p-2 text-gray-400 hover:text-[#3E3B6F] rounded-lg">
                        <RotateCcw size={16} />
                      </button>
                      <button className="p-2 text-gray-400 hover:text-[#3E3B6F] rounded-lg">
                        <Save size={16} />
                      ...
                      <div className="border border-gray-100 rounded-3xl overflow-hidden flex flex-col h-full bg-gray-50">
                        ...
                        <div className="px-4 py-2 border-b border-gray-100 bg-white flex items-center gap-4">
                          <div className="flex gap-1 border-r pr-4">
                            <button className="p-1.5 hover:bg-gray-50 rounded text-gray-500">
                              <Bold size={16} />
                            </button>
                            <button className="p-1.5 hover:bg-gray-50 rounded text-gray-500">
                              <Italic size={16} />
              ...
              <div className="w-80 space-y-6 shrink-0">
                  ...
                  <div className="relative z-10 space-y-4">
                    ...
                    <div className="bg-white/10 p-4 rounded-2xl border border-white/10">
                      ...
                      <p className="text-sm font-bold">
                        {selectedEvent.defaultRecipients}
                      </p>
                    </div>
                    <p className="text-[11px] text-white/60 leading-relaxed italic">
                      "Dynamic mapping based on request ownership and
                      hierarchical structures."
                    </p>
                  </div>
                  <Database
                    size={150}
                    className="absolute -bottom-12 -right-12 text-white/5 rotate-12"
                  />
                </div>
                ...
                <div className="p-6 bg-indigo-50/50 rounded-[32px] border border-indigo-100 flex items-start gap-4">
                  ...
                  <p className="text-[10px] text-indigo-800 leading-relaxed font-bold uppercase">
                    Templates support localization. Add Pakistan/Urdu
                    translations in Language Settings.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-12 space-y-6">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400 max-w-xs mx-auto mt-2 leading-relaxed">
                Choose an event from the library on the left to configure its
